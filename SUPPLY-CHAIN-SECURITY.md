...
**Known gap:** the GitHub Actions in the app workflows + the supply-chain jobs in [`.github/workflows/supply-chain.yml`](./.github/workflows/supply-chain.yml) and [`.github/workflows/snyk-security.yml`](./.github/workflows/snyk-security.yml) are SHA-pinned, but the `gitleaks` job in [`.github/workflows/supply-chain.yml`](./.github/workflows/supply-chain.yml) still uses an unpinned `actions/setup-go@v6` (its `actions/checkout` is SHA-pinned). Audit SHA pins periodically — at minimum once per major release of each action.

### 6. Audit in CI

Three supply-chain jobs in [.github/workflows/supply-chain.yml](./.github/workflows/supply-chain.yml) run on every PR and push to `main` / `staging`:
...
- [x] Bump direct deps with known CVEs: `axios` `1.2.1` → `1.13.5` (CVE-2023-45857, CVE-2024-39338, plus subsequent), `node-fetch` `2.0.0` → `2.7.0` (CVE-2020-15168, CVE-2022-0235).
...
- [x] Pin transitive `axios` consumers in the production tree (originally `@balancer-labs/sdk`'s `^0.24.x` and others) to `1.13.5` via the global `"axios": "1.13.5"` Yarn `resolutions` entry. Clears CVE-2023-45857 / CVE-2024-39338. An earlier path-scoped `"@balancer-labs/sdk/axios": "1.7.7"` was superseded by the global resolution when the direct `axios` dep was bumped further to `1.13.5`. The remaining `axios@^0.21.x` / `axios@1.6.8` transitive entries are reached only through `cypress` and `nx` (devDependencies) and are out of scope for `yarn audit --groups dependencies`.
- [x] Pin transitive `form-data` to `2.5.5` via a top-level Yarn `resolutions` entry (`"form-data": "2.5.5"`). Clears CVE-2024-42459 (affects `< 2.5.5` / `< 3.0.4` / `< 4.0.4`). Path-scoped resolutions (`"request/form-data"`, `"@cypress/request/form-data"`) were tried first but Yarn 1.x's `parent/child` resolution only hits when the parent is close to the root, which `request` isn't (it reaches the tree through six transitive hops via `web3 → web3-bzz → swarm-js → eth-lib → servify`). All `form-data` consumers — `axios`, `jsdom`, `cypress`, `@balancer-labs/sdk#graphql-request`, `nx`, `web3`, `@binance/w3w-*`, etc. — now collapse to `form-data@2.5.5`. API is compatible across `form-data` `2.x` / `3.x` / `4.x`, so downgrading the `^3`/`^4` consumers to `2.5.5` is safe in practice. Yarn warns `"Resolution field \"form-data@2.5.5\" is incompatible with requested version \"form-data@^4.0.0\""` on install — that warning is expected and is just Yarn announcing the intentional override.
- [x] **Full `yarn audit` sweep.** Starting point was 137 unique advisories (6 critical, 69 high) — the stock CI gate would have blocked every PR. Cleared by bumping direct `axios` `1.7.7` → `1.13.5`, `lodash` `4.17.21` → `4.18.1`, and adding Yarn `resolutions` for ~28 transitive packages (`cipher-base`, `elliptic`, `pbkdf2`, `protobufjs`, `sha.js`, `@remix-run/router`, `@xmldom/xmldom`, `body-parser`, `braces`, `cross-spawn`, `defu`, `h3`, `immutable`, `lodash-es`, `node-fetch`, `node-forge`, `parse-duration`, `path-to-regexp`, `picomatch`, `serialize-javascript`, `socket.io-parser`, `svgo`, `tar`, `tiny-secp256k1`, `base-x`, `minimatch`, `secp256k1`, `undici`, `ws`). After the sweep: 0 critical, 3 high — all 3 allowlisted in [`.supply-chain/audit-allowlist.json`](./.supply-chain/audit-allowlist.json) with reasons and 3-month review dates.
  ...
  **Notes on resolution decisions:**
  ...
  - `tar 4 → 7`, `undici 5 → 7`, `serialize-javascript 6 → 7` are all major-version bumps from what consumers request. Same risk class as `minimatch@9` / `glob@10` above, but verified safe in this tree by `yarn nx run-many --target=build --all` (all 12 projects build clean) — no consumer reaches a removed API. Consumer paths: `swarm-js@^0.1.40` → `tar` (via `web3 → web3-bzz`); `@vercel/blob@0.23.4` and `dns-over-http-resolver@^2.1.0` → `undici`; `copy-webpack-plugin@^10.2.4` and `terser-webpack-plugin@^5.3.10` → `serialize-javascript`. Re-run the build sweep on any future bump of these resolutions.
- [x] **Follow-up audit sweep (2026-05).** Cleared 12 newly-published high-severity advisories that landed against versions the tree was already pinned to:
  - **7× Next.js** (GHSA-8h8q-6873-q5fj, GHSA-mg66-mrh9-m8jx, GHSA-c4j6-fc7j-m34r, GHSA-492v-c6pp-mqqv, GHSA-267c-6grr-h53f, GHSA-36qx-fr4f-26g5, GHSA-26hh-7cqf-hhc6 — App Router segment-prefetch / Pages Router i18n middleware bypasses, Cache Components DoS, WebSocket SSRF) cleared by direct bump `next 16.2.4 → 16.2.6`.
  - **4× `protobufjs`** (GHSA-66ff-xgx4-vchm, GHSA-75px-5xx7-5xc7, GHSA-jvwf-75h9-cwgg, GHSA-685m-2w69-288q — code injection via bytes-field defaults, code-gen gadget after prototype pollution, DoS via unsafe option paths, DoS via unbounded recursion) cleared by bumping the existing resolution `protobufjs 7.5.5 → 7.5.6`.
  - **1× `@babel/plugin-transform-modules-systemjs`** (GHSA-fv7c-fp4j-7gwp — arbitrary code generation on malicious input) cleared by adding a new resolution `"@babel/plugin-transform-modules-systemjs": "7.29.4"`. Reaches the tree only at build time through `@nx/next > @nx/js > @babel/preset-env`; `@nx/next` is pinned at the workspace-wide `v17.2.5` so a global resolution is the right tool. The repo only compiles its own source, so reachability of the vulnerability is essentially nil — bumped anyway because the patch is free.

  All three are patch-version bumps with no consumer-facing API change. Verified by full build sweep (`yarn nx run-many --target=build --all`).
- [x] **Follow-up audit sweep (2026-06).** Cleared 5 newly-published high-severity advisories in the production tree, all via patch/minor Yarn `resolutions` bumps (no consumer-facing API change; verified by full build sweep `yarn nx run-many --target=build --all` — all 12 projects build clean):
  - **`ws`** `8.20.1` → `8.21.0` (GHSA-96hv-2xvq-fx4p — memory-exhaustion DoS from tiny fragments/chunks). Reaches the tree via the WalletConnect stack (`wagmi > @wagmi/connectors > … > @walletconnect/jsonrpc-ws-connection > ws`); the pre-existing global `ws` pin already collapses 7.x/8.x consumers onto one version.
  - **`tmp`** `0.2.6` → `0.2.7` (GHSA-7c78-jf6q-g5cm — type-confusion `_assertPath` bypass / path traversal). Build-time only (`@nx/next > @nx/devkit > tmp`).
  - **`form-data`** `2.5.5` → `2.5.6` (GHSA-hmw2-7cc7-3qxx — CRLF injection via unescaped multipart field names/filenames). The existing global `form-data` pin (see the 2.5.5 entry above) bumped one patch.
  - **`hono`** new resolution `4.12.25` (GHSA-88fw-hqm2-52qc — CORS middleware reflects any Origin with credentials when `origin` defaults to wildcard). Reaches the tree via `wagmi > @wagmi/connectors > porto > hono` (`^4.10.3`); patch-compatible.
  - **`protobufjs`** `7.5.8` → `7.6.4` (GHSA-wcpc-wj8m-hjx6 — DoS via unbounded `Any` expansion during JSON conversion). Stayed on the 7.x line (consumers: `ipfs-http-client > … > protobufjs`, `@solana/web3.js`).
...
- [x] SHA-pin `actions/checkout`, `actions/setup-node`, and `actions/cache` in all six app workflows (`build`, `contribute`, `govern`, `launch`, `marketplace`, `operate`) to match the pattern already used in [snyk-security.yml](./.github/workflows/snyk-security.yml) and the supply-chain jobs in [supply-chain.yml](./.github/workflows/supply-chain.yml). Audit the pins periodically — at minimum once per major release of each action — until a bot with `pinDigests: true` is in place.
- [ ] SHA-pin the `actions/setup-go` step in the `gitleaks` job in [.github/workflows/supply-chain.yml](./.github/workflows/supply-chain.yml) (currently `@v6`; `actions/checkout` is already pinned) — same rationale as the app workflows above.

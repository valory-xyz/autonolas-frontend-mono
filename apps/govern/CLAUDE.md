...
- The helper throws if `Dispenser.mapChainIdDepositProcessors(chainId)` is the zero address, i.e. governance has not yet wired the chain's deposit processor (true for 4663 at the time of writing: `robinhoodDepositProcessorL1Address` `0xb9DfcC6155Ba4F211DCf8e6eCc9976Be11bB7a77` exists but is not set on the Dispenser). Claims for such nominees surface that error instead of reverting on-chain.

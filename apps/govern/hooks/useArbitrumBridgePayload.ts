...
export const getArbitrumBridgePayload = async (
  ...
  const l2Provider = new ethersV5.providers.JsonRpcProvider(RPC_URLS[ARBITRUM_CHAIN_ID]);
  ...
  const l1DepositProcessorAddress: string =
    await dispenserContract.mapChainIdDepositProcessors(ARBITRUM_CHAIN_ID);
  ...
  const innerData = ethersV5.utils.defaultAbiCoder.encode(
    ...
    [stakingTargets, dummyAmounts, dummyBatchHash],
  );
  ...
  // 3 ABI-encoded words (maxSubmissionCost, extraData offset, extraData length=0)

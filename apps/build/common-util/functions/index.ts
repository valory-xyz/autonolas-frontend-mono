...
export const getIsValidChainId = (chainId: number) => getIsValidChainIdFn(SUPPORTED_CHAINS, chainId);
...
export const sendTransaction = (fn: any, account: string) => sendTransactionFn(fn, account, {
...
export const getDispenserContract = () => {
  ...
  const contract = getContract(DISPENSER.abi, DISPENSER.addresses[chainId as keyof typeof DISPENSER.addresses]);
  return contract;
};

export const getTreasuryContract = () => {
  const { chainId } = getWeb3Details();
  const contract = getContract(TREASURY.abi, TREASURY.addresses[chainId as keyof typeof TREASURY.addresses]);
  return contract;
};

export const getTokenomicsContract = () => {
  const { chainId } = getWeb3Details();
  const contract = getContract(TOKENOMICS.abi, TOKENOMICS.addresses[chainId as keyof typeof TOKENOMICS.addresses]);
  return contract;
};
...
export const getAgentContract = () => {
  ...
  const contract = getContract(
    AGENT_REGISTRY.abi,
    AGENT_REGISTRY.addresses[chainId as keyof typeof AGENT_REGISTRY.addresses],
  );
  return contract;
};
...
export const getComponentContract = () => {
  ...
  const contract = getContract(
    ...
    COMPONENT_REGISTRY.addresses[chainId as keyof typeof COMPONENT_REGISTRY.addresses],
  );
  return contract;
};

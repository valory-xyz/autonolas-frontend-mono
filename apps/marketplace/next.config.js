...
const nextConfig = {
  ...
  // `/[network]/ai-agents/mint` was matching `[id].jsx` with id="mint"
  // instead of the static `mint.jsx` page). `distDir` is computed by
  // `@nx/next/plugins/with-nx` from the build target's `outputPath` in
  // `project.json` — don't set it here, or withNx will double-join the paths.
  i18n: {
    locales: ['en'],
    defaultLocale: 'en',
  },

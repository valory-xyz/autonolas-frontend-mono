...
Discover, register, deploy, and interact with **mechs** (autonomous AI agents) and **registry** entities (agents, components, services) across multiple chains. Users connect a wallet (Reown AppKit / WalletConnect) to register, deploy, or use mechs. Multi-chain: Ethereum, Gnosis, Polygon, Arbitrum, Optimism, Base, Mode, Celo. Also supports **Solana (SVM)**.
...
- **Wallet**: Reown AppKit (Wagmi adapter) for EVM chains; Solana wallet adapter for SVM. See `pages/_app.tsx`, `common-util/Login/LoginV2.jsx`, `common-util/hooks/useSvmConnectivity.jsx`, `common-util/hooks/useHelpers.tsx`.
...
- **Reown AppKit / WalletConnect is required** for core flows (registration, deployments, transactions).

...
import { erc20Params, tokenomicsParams, uniswapV2PairParams } from 'common-util/Contracts/params';
...
/**
 * The bonding-products pipeline as plain functions, so the server fetch (`fetchBondingProducts`)
 * and the `useProducts` hook compose exactly the same steps. Everything browser- or wallet-bound
 * is injected through `deps` - `chainId`, a `multicall`, `getCurrentPriceWhirlpool` - and nothing
 * here imports React, wagmi hooks or the Solana SDK, so the ISR function does not pay for them.
 */

/** The Orca whirlpool position for the OLAS-WSOL LP; `TokenManagement/constants` builds the PublicKey from it. */
export const SVM_POSITION_ADDRESS = 'EHQbFx7m5gPBqXXiViNBfHJDRUuFgqqYsLzuWu18ckaR';
...
const LP_PAIRS = {
  ...
  '0x3685B8cC36B8df09ED9E81C1690100306bF23E04': {
    ...
    name: 'OLAS-WSOL',
    originAddress: SVM_POSITION_ADDRESS,
    dex: DEX.SOLANA.name,
 ...
 */
// `memoize` keys on the first argument only: cached by address, for the life of the process -
// the chain is constant per runtime, and a not-found `tokenSymbol: null` is cached too.
 ...
 */
// Cached by token address (see `getLpTokenDetails`).
...
const getCurrentPriceUniswapFn = memoize(async (tokenAddress) => {
  // Same case-insensitive lookup as `getLpTokenDetails`, so a lowercase address resolves too.
  const lpPairKey = Object.keys(LP_PAIRS).find((key) => areAddressesEqual(key, tokenAddress));
  const chainId = lpPairKey ? LP_PAIRS[lpPairKey].lpChainId : undefined;
  if (!chainId) {
    throw new Error(`Chain id not found for provided token address: ${tokenAddress}`);
  }

  const tokenOriginAddress = LP_PAIRS[lpPairKey].originAddress;
    ...
    if (productList[i].token === ADDRESS_ZERO) {
      ...
      if (isL1Network(lpChainId)) {
        ...
        if (dex === DEX.UNISWAP.name || dex === DEX.UBESWAP.name) {
          currentLpPrice = getCurrentPriceUniswapFn(productList[i].token);
          otherRequests[i] = currentLpPrice;
        } else if (dex === DEX.BALANCER.name) {
          currentLpPrice = getCurrentPriceBalancerFn(productList[i].token, chainId);
          otherRequests[i] = currentLpPrice;
...
export const addSupplyLeftToProducts = async (
  ...
  list.map((product) => {
    ...
    // Products 0-9 have two close rows on the subgraph, one of them with a null supply that
    // `toBigInt` rejects; `find` must skip it or the real row is shadowed and the list throws.
    const closeProductEvent = closedProductEvents?.find(
      (event) => event.productId === `${product.id}` && event.supply != null,
    );
 ...
 */
/** The steps above composed; shared by `useProducts` and the server fetch. `deps`: see `addCurrentLpPriceToProducts`. */

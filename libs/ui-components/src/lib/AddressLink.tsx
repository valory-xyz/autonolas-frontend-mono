...
export const getIpfsUrl = (hash: string) => {
  ...
  // Multibase-encoded CIDs (base32 CIDv1 → 'bafy…', base58 CIDv0 → 'Qm…')
  // are usable at the gateway as-is; adding HASH_PREFIX would double-encode.
  if (/^(bafy|Qm)/.test(hash)) return `${GATEWAY_URL}${hash}`;
